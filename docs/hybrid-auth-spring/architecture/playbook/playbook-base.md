...
- **The flow scales by tier (§21):**
  - **prototype** — `feat/<NNN>-<slug>` off `dev` → **PR to `dev`**. No epic branch.
  - **small / medium** — features merge **locally** into an `epic/<NNN>-<slug>` branch (the cohesive batch); the **epic is the PR unit to `dev`** (one `dev` PR per epic).
...
> **PR unit by tier (§16.2).** At prototype the loop targets `dev` with a `feat/<NNN>-<slug>` branch. At **small+** the PR unit is the **`epic/<NNN>-<slug>`** branch — features merge **locally** into the epic (no push-gate), and the loop below runs once at the **epic → `dev`** boundary (read the epic branch for the branch name in the steps).
